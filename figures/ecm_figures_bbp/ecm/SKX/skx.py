...
class base_skx():
    def __init__(self):
        ...
        #self.mem_latency = 25.6 # cy per access, cheat from bench
        self.cpu_f    = 2.3 # GHz
        self.max_nthreads = 18.
        self.scalar_exp_thruput = 15.1 #cy
        self.scalar_exp_latency = 22.2 #cy
    ...
    def TL1L2(self, kernel):
        latency_bound = kernel['latency_bound'] if 'latency_bound' in kernel else False
        if not latency_bound:
            return 8.*(kernel['reads'] + 2.*kernel['writes'])/self.L1L2_BW
        else:
    ...
    def TL2L3(self, kernel):
        latency_bound = kernel['latency_bound'] if 'latency_bound' in kernel else False
        if not latency_bound:
            #return 8.*(kernel['reads'] + 2.*kernel['writes'])/self.L2L3_BW
            return 8.*(kernel['reads'] + kernel['writes'])/self.L2L3_BW # accounts for victim L3
        else:
            return (kernel['accesses'])*self.L3_latency
    ...
    def rate_limiting_steps(self, kernel):
        ...
        if self.TOL(kernel) > self.TnOL(kernel) + self.TL1L2(kernel) + self.TL2L3(kernel) + self.TMem(kernel):
