COMMENT
/**
 * @file ProbFiltAMPANMDA_EMS.mod
 * @brief added linear dendritic filter function with w_corr, tau_corr (roessert)
 * @author king, muller, reimann, ramaswamy, roessert
 * @date 2015-06-22
...
NEURON {
    THREADSAFE
    POINT_PROCESS ProbFiltAMPANMDA_EMS
    RANGE tau_r_AMPA, tau_d_AMPA, tau_r_NMDA, tau_d_NMDA
    RANGE Use, u, Dep, Fac, u0, mg, Rstate, tsyn_fac, u
    RANGE i, i_AMPA, i_NMDA, g_AMPA, g_NMDA, g, e, NMDA_ratio, AMPA_ratio
    RANGE w_corr, tau_corr
...
STATE {
    ...
    isoma        : surrogate synaptic current at the soma (after filtering)
}
...
DERIVATIVE state {
:printf( "A_AMPA %f B_AMPA %f A_NMDA %f B_NMDA %f isoma %f tau_corr %f\n", A_AMPA, B_AMPA, A_NMDA, B_NMDA, isoma, tau_corr)
    A_AMPA' = A_AMPA/ tau_r_AMPA
    B_AMPA' = B_AMPA/ tau_d_AMPA
    A_NMDA' = A_NMDA/ tau_r_NMDA
    B_NMDA' = B_NMDA/ tau_d_NMDA
    isoma' = idend + (isoma - idend)/tau_corr
:printf("Finished computing states\n")
}


NET_RECEIVE (weight,weight_AMPA, weight_NMDA, Psurv, tsyn (ms)){
    LOCAL result
    weight_AMPA = weight * AMPA_ratio
...
ENDVERBATIM
    ...
    if (Rstate == 1) {
        ...
        if (result<u) {
            ...
            if( verboseLevel > 0 ) {
                printf( "Release! %f at time %g: vals %g %g %g %g, u:%g\n", synapseID, t, A_AMPA, weight_AMPA, factor_AMPA, weight, u )
            }

        }
        else {
            if( verboseLevel > 0 ) {
                printf("Failure! %f at time %g: urand = %g, u:%g\n", synapseID, t, result, u )
            }
        }
    }
}
