...
NEURON {
    THREADSAFE
    POINT_PROCESS ProbFiltGABAAB_EMS
    RANGE tau_r_GABAA, tau_d_GABAA, tau_r_GABAB, tau_d_GABAB
    RANGE Use, u, Dep, Fac, u0, Rstate, tsyn_fac, u
    RANGE i,i_GABAA, i_GABAB, g_GABAA, g_GABAB, g, e_GABAA, e_GABAB, GABAB_ratio, GABAA_ratio
...
NET_RECEIVE (weight, weight_GABAA, weight_GABAB, Psurv, tsyn (ms)){
    LOCAL result
    weight_GABAA = weight*GABAA_ratio
...
ENDVERBATIM
    ...
    if (Rstate == 1) {
        ...
        if (result<u) {
            ...
            if( verboseLevel > 0 ) {
                printf( "Release! %f at time %g: vals %g %g %g %g, u:%g\n", synapseID, t, A_GABAA, weight_GABAA, factor_GABAA, weight, u )
            }
